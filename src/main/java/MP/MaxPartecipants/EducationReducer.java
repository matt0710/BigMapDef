...
public class EducationReducer extends Reducer<Text, IntWritable, Text, IntWritable> {
    ...
    public void cleanup (Context context) throws IOException, InterruptedException {
        ...
        Collections.sort(partecipants, Collections.reverseOrder()); //we first put the map values into an arrayList and then we sort
        //in descending order the latter.

        if (partecipants.size() < 5) //we do this check in order to avoid IndexOutOfBoundException
            for (int i=0; i<partecipants.size(); i++) {
                for (Map.Entry<String,Integer> entry : map.entrySet()) {
                    if (partecipants.get(i) == entry.getValue()) { //if the entry value is equal to the ith-item in the ordered list, we
